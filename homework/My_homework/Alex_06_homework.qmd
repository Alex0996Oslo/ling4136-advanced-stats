...
#Discription: for both conditions, the semantically similar names and semantically different names, the highest concentration of data points appeared at around 75. However, the majority of the data points for semantically similar condition appeared to be before 75, while the semantically different condition has more even distribution, with slightly more datapoints scaled to the tail than head.
```
...
#I'm not sure i did this one right...
...
my_priors <- c(prior(normal(74, 34.1), class = "Intercept"),
               prior(normal(0, 34.1), class = "b"))

#Again, I'm not sure what's a reasonable number to put into this, I'll just use the output of get_prior...
```
...
# Compared to the observed data, the posterior samples are a bit mor espread out and have a lower "peak", which could indicate that my model has a somehow higher variability than the data actually is. 
```
