...
my_priors <- c(
  ...
  prior(normal(6.5, 2.5), class = "Intercept")
)
...
posterior <- as_draws_df(xmdl_sqrt)
posterior[, c("b_Intercept", "b_log_frequency")]
CI <- quantile(posterior$b_log_frequency, c(.025, .975))

library(ggplot2)

plot <- data.frame(
  log_frequency = seq(
    min(quantese$log_frequency),
    max(quantese$log_frequency),
    length.out = 100
  )
)

pred <- fitted(xmdl_sqrt, 
               newdata = plot, 
               re_formula = NA,
               probs = c(0.025, 0.975))

plot <- cbind(plot, pred)

ggplot(plot, aes(x = log_frequency, y = Estimate)) +
  geom_line() +
